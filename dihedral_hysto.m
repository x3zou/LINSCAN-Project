...
%          [fm]=sdr2mij([s3(i) d3(i) r3(i)],deg2rad);
%          focalmech(fm, mean([x(i,1) x(i,3)])-4,mean([x(i,2) x(i,4)])-4, 3, 'r','dc'), hold on
%          line([x(j,1) x(j,3)],[x(j,2) x(j,4)],'lineStyle','-','lineWidth',2,'Color','b'), hold on
% 
%          text(mean([x(j,1) x(j,3)])-2,mean([x(j,2) x(j,4)])-2, ['strike: ' num2str(s3(j))]), hold on
%          text(mean([x(j,1) x(j,3)])-2,mean([x(j,2) x(j,4)])-2.5, ['rake: ' num2str(r3(j))]), hold on
%          [fm]=sdr2mij([s3(j) d3(j) r3(j)],deg2rad);
%          focalmech(fm, mean([x(j,1) x(j,3)])-4,mean([x(j,2) x(j,4)])-4, 3, 'b','dc'), hold on
% 
%          axis equal;
%          pause         
%       end
...
xlim=[20 110];
%ax=gca;
%set(ax,'xlim',xlim,'box','off','FontSize',Fs), hold on
%set(gca,'xlim',xlim,'ylim',[0 180],'FontSize',Fs), hold on
%set(gca,'xlim',xlim,'ylim',[0 40],'FontSize',Fs), hold on
yl=get(gca,'ylim');
xi=30;
...
yyaxis right
x1=20:90;
%plot(x,0.5*atan(1./(x/2/180*pi))), hold on
