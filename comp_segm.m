...
if isfile(f)
 ...
 for j=1:length(c(:,1))
  ...
  line(xu(ind),yu(ind),'lineStyle','none','Marker','.','MarkerSize',1,'MarkerFaceColor',grey,'MarkerEdgeColor',grey), hold on
  for i=1:m
    line([x(i,1) x(i,3)],[x(i,2) x(i,4)],'lineStyle','-','Color','r','LineWidth',2), hold on
  end
   line([x(c(j,1),1) x(c(j,1),3)],[x(c(j,1),2) x(c(j,1),4)],'lineStyle','-','Color','b','LineWidth',2), hold on
   text(x(c(j,1),1),x(c(j,1),2),num2str(c(j,1),'%d\n')), hold on
   line([x(c(j,2),1) x(c(j,2),3)],[x(c(j,2),2) x(c(j,2),4)],'lineStyle','-','Color','b','LineWidth',2), hold on
   text(x(c(j,2),1),x(c(j,2),2),num2str(c(j,2),'%d\n')), hold on
%   pause
 end
end
