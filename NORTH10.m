%This is the algorithms for plotting individual clusters with strike of +-
%10 degrees North
%Xiaoyu Zou, x3zou@ucsd.edu, 7/8/2022
clear
clc
...
numeral=0;
newpointsdr=zeros(1,7);
whatstrike=zeros(1,1);
newends=zeros(1,4);
for j=1:length(ends)
    x1=ends(j,1);
    x2=ends(j,3);
    ...
    whatstrike=[whatstrike;strike];
    if strike>100 || strike<80
        continue
    end
    ...
    numeral=numeral+1;
    no=numeral*ones(size(pp,1),1);
    pp=[no pp];
    newpointsdr=[newpointsdr;pp];
    newends=[newends;[x1 y1 x2 y2]];
end
newpointsdr(1,:)=[];
newends(1,:)=[];
...
m=3;
n=3;
p=1;%page number for subplots
j=0+(p-1)*m*n;
ha = tight_subplot(m,n,[0.03 0.03],[0.1 0.01],[0.01 0.01]);
for i=1:m*n
    axes(ha(i));
    j=j+1;
    pp=horzcat(newpointsdr(find(newpointsdr(:,1)==j),2),newpointsdr(find(newpointsdr(:,1)==j),3),newpointsdr(find(newpointsdr(:,1)==j),4),newpointsdr(find(newpointsdr(:,1)==j),5),newpointsdr(find(newpointsdr(:,1)==j),6),newpointsdr(find(newpointsdr(:,1)==j),7));
    x1=newends(j,1);
    x2=newends(j,3);
    y1=newends(j,2);
    y2=newends(j,4);
    ...
    %     text((x1+x2)/2,(y1+y2)/2,sprintf('n=%i,d=%f,r=%f,l=%f',num,mean_dist,cc,L),'Color','r','FontWeight','bold','Fontsize',8);
end
