...
unix('cp eqn1.ross eqc1.ross');
NumIter=4; %number of iterations
dist = 0.07; % distance threshold, km
del=0.015;
...
load eqc1.ross; % iteratively selected aftershocks (remove outliers)
x=eqc1;
...
dist = dist-del; % distance threshold, km
s=find(perp < dist);
x=x1(s); 
y=y1(s); 
