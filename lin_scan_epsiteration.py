...
if __name__ == '__main__':
    ...
    # read data
    dataset=np.loadtxt('d2eps=16_input_2.txt',delimiter=',')
    ...
    temp = np.array([dataset[i, :] for i in range(len(dataset)) if typelist[i] != -1])
    temp_list = np.array([typelist[i] for i in range(len(typelist)) if typelist[i] != -1])
    np.savetxt('d2iter_eps=sqrt16_2.txt',temp)
    ...
    ax.set_ylabel('km')
    #ax.set_xlim(-70,100)
    ax.set_title('0.2,d2eps=sqrt16_2')
    np.savetxt('d2list_eps=sqrt16_2.txt',temp_list)
    

    plt.scatter(temp[:, 0], temp[:, 1], c=temp_list, cmap='jet', marker='.', s=3)
    plt.savefig('d2iter_eps=sqrt16_2.png')
    plt.show()
