#modified by Xiaoyu Zou x3zou@ucsd.edu 10/2/2021
import numpy as np
...
if __name__ == '__main__':
    ...
    # read data
    dataset=np.loadtxt('sqrt16_input.txt',delimiter=',')
    ...
    # x_range = [-1, 1]
    # y_range = [-1, 1] #changed to [-1,1] to work with all the data
    ...
    #Standard parameters from dbscan   
    eps = np.sqrt(16)#original value:sqrt(10)
    min_pts = 10#original value:10
    
    #lower bound on the correlation of clusters we will accpet.
    threshold = 0.2
    
    #number of neighbours used to estimate the local neiborhood of each point.
    #smaller value increases noise, larger value smooth out noice, but reduce the impact of local direciton
    ecc_pts = 15#original value:15
    ...
    plt.scatter(dataset[:, 0], dataset[:, 1], c=typelist, cmap='jet', marker='.',s=3)
    ...
    plt.scatter(temp[:, 0], temp[:, 1], c=temp_list, cmap='jet',marker='.', s=3)
    plt.show()
    ...
    temp = np.array([dataset[i, :] for i in range(len(dataset)) if typelist[i] != -1])
    temp_list = np.array([typelist[i] for i in range(len(typelist)) if typelist[i] != -1])
    np.savetxt('correlation0.2iter_eps=sqrt16.txt',temp)
    ...
    ax.set_ylabel('km')
    ax.set_xlim(-70,100)
    ax.set_title('0.2,eps=sqrt16')
    np.savetxt('02list_eps=sqrt16.txt',temp_list)
    

    plt.scatter(temp[:, 0], temp[:, 1], c=temp_list, cmap='jet', marker='.', s=3)
    plt.savefig('correlation02iter_eps=sqrt16.png')
    plt.show()
